...
- [ ] **Multiple Sources of Truth**: User data exists in both auth.db and forums.db with potential inconsistencies
- [ ] **Session Fragmentation**: 44 sessions in auth.db, 36 in forums.db - unclear which are valid
- [ ] **Password Hash Inconsistency**: Same usernames may have different password hashes across databases
- [ ] **Missing Central Authority**: No single source for authentication decisions

### 2. Session Security Concerns
- [ ] **Orphaned Sessions**: Sessions may exist without corresponding user records
- [ ] **Cross-Database Sessions**: Sessions created in one DB may not be recognized in another
- [ ] **Session Migration**: Must ensure all valid sessions remain functional post-migration
- [ ] **Token Validation**: JWT/session tokens must continue working without re-authentication
...
- [ ] **Connection Limits**: Implement proper connection pooling (max 5-10 connections)

## Pre-Migration Checklist

### Data Validation
- [ ] Verify all user records have required fields (username, email, password_hash)
