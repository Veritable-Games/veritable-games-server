...
async function main() {
  ...
  if (!connectionString) {
    ...
    console.error('Usage: POSTGRES_URL=postgresql://... node scripts/wiki/analyze-journal-entries.js');
    process.exit(1);
  }
  ...
  try {
    ...
    console.log(`Found ${journalLikeUncategorized.length} journal-like entries in Uncategorized:\n`);
    ...
    if (journalLikeUncategorized.length > 0) {
      ...
      journalLikeUncategorized.forEach((page, idx) => {
        ...
        console.log(`     Created: ${page.created_at}, User: ${page.created_by}, Namespace: ${page.namespace}`);
      });
      console.log();
    }
    ...
    console.log(`Found ${duplicates.length} potential duplicates (same slug in both categories):\n`);
    ...
    if (duplicates.length > 0) {
      ...
      duplicates.forEach((page, idx) => {
        ...
        console.log(`     Uncategorized: ID ${page.id} - "${page.title}" (created: ${page.created_at})`);
        console.log(`     Journals: ID ${journalVersion.id} - "${journalVersion.title}" (created: ${journalVersion.created_at})`);
      });
      console.log();
    }
    ...
    if (journalAuthors.size > 0) {
      ...
      authors.forEach(author => {
        ...
        console.log(`  • ${author.username} (ID ${author.id}): ${journalCount} in Journals, ${uncatCount} in Uncategorized`);
      });
      console.log();
    }
