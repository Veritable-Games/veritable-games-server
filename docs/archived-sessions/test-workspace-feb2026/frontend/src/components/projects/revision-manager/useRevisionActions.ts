...
export function useRevisionActions({
  ...
  const deleteRevision = async (revisionId: number) => {
    ...
    try {
      await fetchJSON(
        `/api/projects/${encodeURIComponent(projectSlug)}/revisions?revisionId=${revisionId}`,
        {
          method: 'DELETE',
        }
      );
  ...
  const restoreRevision = async (revision: Revision) => {
    ...
    setIsProcessing(true);
    try {
      await fetchJSON(`/api/projects/${encodeURIComponent(projectSlug)}/revisions/restore`, {
        method: 'POST',
        body: {
