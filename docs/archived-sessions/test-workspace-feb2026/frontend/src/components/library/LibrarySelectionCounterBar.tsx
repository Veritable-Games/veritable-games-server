...
/**
 * Library Selection Counter Bar Component
 * Displays count of selected documents with action buttons
 * Only appears when documents are selected (Ctrl+Click or Ctrl+A)
 */

interface LibrarySelectionCounterBarProps {
  selectedCount: number;
  onClearSelection: () => void;
  onSelectAll?: () => void;
}

export function LibrarySelectionCounterBar({
  selectedCount,
  onClearSelection,
  onSelectAll,
}: LibrarySelectionCounterBarProps) {
      ...
      <div className="mx-auto flex max-w-7xl items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div className="font-semibold text-white">
            <span className="text-lg text-blue-300">{selectedCount}</span>
            <span className="ml-2">
