...
export const POST = withSecurity(async (request: NextRequest) => {
  try {
    ...
    // Hard ban users (permanent)
    const userService = new UserService();
    await userService.batchHardBanUsers(numericUserIds, user.id, reason);

    return NextResponse.json({
      success: true,
      message: `Successfully hard-banned ${userIds.length} user(s) permanently`,
    });
  } catch (error) {
