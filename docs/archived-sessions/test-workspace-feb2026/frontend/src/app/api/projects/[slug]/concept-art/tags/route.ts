...
/**
 * GET /api/projects/[slug]/concept-art/tags
 * Get all available concept art tags for this project
 */
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const result = await projectGalleryService.getAllTags('concept-art', project.id);
...
/**
 * POST /api/projects/[slug]/concept-art/tags
 * Create a new concept art tag (admin only)
 */
async function createTagHandler(
  ...
  try {
    ...
    const categoryResult = await dbAdapter.query(
      'SELECT id FROM concept art_categories WHERE name = $1',
      ['General'],
      { schema: 'content' }
    );
    ...
    if (defaultCategory) {
      ...
      const insertResult = await dbAdapter.query(
        `INSERT INTO concept art_categories (id, name, description, visibility, display_order)
         VALUES (gen_random_uuid()::text, $1, $2, $3, $4) RETURNING id`,
        ['General', 'Default category for concept art tags', 'public', 0],
        { schema: 'content' }
      );
