...
/**
 * DELETE /api/projects/[slug]/references/tags/[tagId]
 * Delete a reference tag (admin only)
 */
async function deleteTagHandler(
  ...
  try {
    ...
    const tagResult = await dbAdapter.query(
      'SELECT id FROM reference_tags WHERE id = $1',
      [tagId],
      { schema: 'content' }
    );
    ...
    await dbAdapter.query('DELETE FROM project_reference_image_tags WHERE tag_id = $1', [tagId], {
      schema: 'content',
    });

    // Delete the tag itself
    await dbAdapter.query('DELETE FROM reference_tags WHERE id = $1', [tagId], {
      schema: 'content',
    });
