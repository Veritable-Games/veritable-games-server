 ...
 * Get all concept art images for a project with optional filtering
 */
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    logger.error('Error fetching concept art images:', error);
    return NextResponse.json({ error: 'Failed to fetch concept art images' }, { status: 500 });
  }
}

/**
 * POST /api/projects/[slug]/concept-art
 * Upload new concept art images (admin only)
 */
async function uploadConceptArtHandler(
  request: NextRequest,
  ...
  try {
    ...
    // Use config for validation (50MB max, includes PSD support)
    const uploadResults = [];

    for (const file of files) {
      try {
        // Validate file with comprehensive security checks
        // Concept art: lossless storage, no compression
        const validation = await validateImageUpload(file, {
          maxSizeBytes: CONCEPT_ART_CONFIG.maxFileSize,
          allowedMimeTypes: CONCEPT_ART_CONFIG.allowedMimeTypes,
        ...
        const relativePath = `/uploads/concept-art/${slug}/${filename}`;
...
export const POST = withSecurity(uploadConceptArtHandler);
