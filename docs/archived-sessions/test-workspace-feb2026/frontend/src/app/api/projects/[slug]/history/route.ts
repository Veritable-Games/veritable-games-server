 ...
 * Get all history images for a project with optional filtering
 */
export async function GET(request: NextRequest, context: { params: Promise<{ slug: string }> }) {
  try {
    ...
    logger.error('Error fetching history images:', error);
    return NextResponse.json({ error: 'Failed to fetch history images' }, { status: 500 });
  }
}

/**
 * POST /api/projects/[slug]/history
 * Upload new history images (admin only)
 */
async function uploadHistoryHandler(
  request: NextRequest,
  ...
  try {
    ...
    for (const file of files) {
      try {
        ...
        const relativePath = `/uploads/history/${slug}/${filename}`;
        ...
        const result = await projectGalleryService.createImage('history', input, user.id as UserId);
...
export const POST = withSecurity(uploadHistoryHandler);
