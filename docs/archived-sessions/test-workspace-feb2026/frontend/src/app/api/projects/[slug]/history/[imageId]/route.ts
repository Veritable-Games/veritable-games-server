...
/**
 * GET /api/projects/[slug]/history/[imageId]
...
export async function GET(
  ...
  try {
    ...
    logger.error('Error fetching history image:', error);
    return NextResponse.json({ error: 'Failed to fetch history image' }, { status: 500 });
  }
}

/**
 * PATCH /api/projects/[slug]/history/[imageId]
 * Update history image metadata (admin only)
 */
async function updateHistoryHandler(
  request: NextRequest,
  context: { params: Promise<{ slug: string; imageId: string }> }
) {
  try {
...
/**
 * DELETE /api/projects/[slug]/history/[imageId]
 * Soft delete a history image (admin only)
 */
async function deleteHistoryHandler(
  request: NextRequest,
  context: { params: Promise<{ slug: string; imageId: string }> }
) {
  try {
...
export const PATCH = withSecurity(updateHistoryHandler, {});

export const DELETE = withSecurity(deleteHistoryHandler, {});
