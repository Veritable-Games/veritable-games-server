...
export const GET = withSecurity(
  async (request: NextRequest, context: { params: Promise<{ slug: string }> }) => {
    try {
      ...
      logger.info('[Tags Route] GET request for anarchist document:', params.slug);
      ...
      try {
        ...
        logger.info('[Tags Route] Query result:', documentResult.rows.length, 'rows');
      } catch (queryError) {
        logger.error('[Tags Route] Query failed:', queryError);
        throw queryError;
      }
      ...
      if (!document) {
        logger.info('[Tags Route] Document not found for slug:', params.slug);
    ...
    `,
      ...
      // Get all available tags from shared.tags
    ...
    `,
      ...
      logger.error('Failed to fetch anarchist document tags:', error);
    ...
    try {
      ...
      if (tagId !== undefined) {
        ...
        try {
          ...
          const tagResult = await dbAdapter.query('SELECT * FROM tags WHERE id = $1', [tagId], {
            schema: 'shared',
          });
      ...
      logger.error('Failed to add tags to anarchist document:', error);
...
export const DELETE = withSecurity(
  async (request: NextRequest, context: { params: Promise<{ slug: string }> }) => {
    try {
      ...
      logger.error('Failed to remove tag from anarchist document:', error);
