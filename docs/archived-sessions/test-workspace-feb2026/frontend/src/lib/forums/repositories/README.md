...
Comprehensive data access layer for the forum system using the Result pattern
...
Users are stored in `auth.db`, not `forums.db`. The repositories handle this
automatically:
...
8. **Validate input**: Repositories do basic validation, but add more in service
   layer
...
const topics = db
  .prepare('SELECT * FROM topics WHERE category_id = ?')
  .all(categoryId);
