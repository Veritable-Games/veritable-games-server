/**
 * Migration: Create Anarchist Library Schema
 *
 * Adds support for the Anarchist Library Network archive (24,643 texts across 27 languages)
 * Uses PostgreSQL schemas for organization (separate from main library)
 *
 * Hybrid storage approach:
 * - Metadata stored in PostgreSQL for search and filtering
 * - Document content stored on filesystem (Docker volume at /app/anarchist-library)
 *
 * Date: 2025-11-08
 */

-- Create anarchist schema
CREATE SCHEMA IF NOT EXISTS anarchist;
...
CREATE TABLE IF NOT EXISTS anarchist.documents (
  ...
  -- File storage reference (path relative to /app/anarchist-library)
  ...
  -- Organization
  category TEXT,  -- e.g., 'anarchist-en', 'anarchist-de', 'anarchist-fr'
...
-- GRANT USAGE ON SCHEMA anarchist TO [app_role];
-- GRANT SELECT, INSERT, UPDATE, DELETE ON ALL TABLES IN SCHEMA anarchist TO [app_role];
-- GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA anarchist TO [app_role];
