...
- **'unsafe-eval'**: Required for Three.js shader compilation (WebGL shaders use eval-like functions)
- **Removed 'unsafe-inline'**: No longer needed with static module loading

### `worker-src 'self' blob:`

- **'self'**: Web Workers from same origin
- **blob:**: Required for dynamically created worker scripts (orbital-worker.js, stellar-worker.js)
